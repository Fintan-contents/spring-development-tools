...
EOF
    ...
    It "MAIL_OPTとINP_JAVA_OPTを定義しない場合、javaコマンドに渡されるオプションは空となること"
    ...
    It "MAIL_OPTだけ定義してINP_JAVA_OPTは定義しない場合、javaコマンドに渡されるオプションはMAIL_OPTだけとなること"
        global_make_script /home/app/app/shell-common/conf/java_env.config << EOF
MAIL_OPT=-Dfoo=bar
EOF
    ...
    It "MAIL_OPTは定義せずINP_JAVA_OPTだけ定義した場合、javaコマンドに渡されるオプションはINP_JAVA_OPTだけとなること"
    ...
    It "MAIL_OPTとINP_JAVA_OPTの両方を定義し場合、javaコマンドのオプションには両方の値が渡されること"
        global_make_script /home/app/app/shell-common/conf/java_env.config << EOF
MAIL_OPT=-Dfoo=bar
EOF
        export INP_JAVA_OPT=-Dfizz=buzz
